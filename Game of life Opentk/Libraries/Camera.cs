...
namespace Game_of_life_Opentk
{
    class Camera
    {
        ...
        static public bool HasPressed(Key k)
        {
            KeyboardState keystate = Keyboard.GetState();
            if(keystate.IsKeyDown(k) && lastKey.IsKeyUp(k))
            {
                return true;
            }
            return false;
        ...
        public static bool HasClicked(MouseButton ms, bool once)
        {
            if (once)
            {
            ...
            {
                if (mouse.IsButtonDown(ms))
                {
                    lastClickPosition = new Vector2(cursorPos.X, cursorPos.Y);
                    return true;
                }
                else if (mouse.IsButtonUp(ms))
                {
                    lastClickPosition = new Vector2(-1, -1);
                }
            }
